...
t_tocador *criar_tocador()
{
    ...
    novo->proximas = criar_fila_circular(15);
...
void loading_tocador(t_tocador *t)
{
    if (t->status == STOP)
    {
        ...
        if (!(vazia_fila_circular(t->proximas))) // se a fila nao esta vazia
        {
            t->faixa_carregada = primeiro_fila_circular(t->proximas);
            t->status = LOAD;
        }
...
void end_tocador(t_tocador *t)
{
    ...
    {
        t->status = STOP;
        t_musica *m = desenfileirar_fila_circular(t->proximas);
...
{
    if (!vazia_fila_circular(t->proximas))
    {
        t_musica *m = desenfileirar_fila_circular(t->proximas);
        t->faixa_carregada = primeiro_fila_circular(t->proximas);
...
void backward_tocador(t_tocador *t)
{
    if (altura_pilha(t->ultimas) > 0)
    {
        ...
        imprimir_musica(m);
        enfileirar_fila_circular(t->proximas, m);
    }
}
...
static void loading_proximas(t_tocador *t)
{
    ...
    while (status == 1)
    {
        ...
        enfileirar_fila_circular(t->proximas, musica);
        scanf("%d", &status);
    }
}
